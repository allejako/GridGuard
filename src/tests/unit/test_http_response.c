...
{
    print_section("200 OK — JSON-svar");
    ...
    ASSERT("Svar innehåller 'HTTP/1.1 200'",        strstr(buf, "HTTP/1.1 200") != NULL);
    ASSERT("Content-Type är application/json",      strstr(buf, "application/json") != NULL);
    ASSERT("Body innehåller JSON-data",             strstr(buf, "{\"status\":\"ok\"}") != NULL);
}
...
static void test_401_unauthorized(void)
{
    ...
    ASSERT("Svar innehåller 'HTTP/1.1 401'",      strstr(buf, "HTTP/1.1 401") != NULL);
    ASSERT("Body innehåller 'Unauthorized'",       strstr(buf, "Unauthorized") != NULL);
    ASSERT("Content-Type är application/json",     strstr(buf, "application/json") != NULL);
}
...
static void test_404_not_found(void)
{
    ...
    ASSERT("Svar innehåller 'HTTP/1.1 404'",   strstr(buf, "HTTP/1.1 404") != NULL);
    ASSERT("Body innehåller 'Not found'",       strstr(buf, "Not found") != NULL);
}
...
static void test_500_internal_error(void)
{
    ...
    ASSERT("Svar innehåller 'HTTP/1.1 500'",      strstr(buf, "HTTP/1.1 500") != NULL);
    ASSERT("Body innehåller 'Internal error'",     strstr(buf, "Internal error") != NULL);
}

static void test_response_has_content_length(void)
{
    print_section("Content-Length-header finns i alla svar");
    ...
    ASSERT("200-svar har Content-Length", strstr(buf, "Content-Length:") != NULL);
    ...
    ASSERT("401-svar har Content-Length", strstr(buf, "Content-Length:") != NULL);
}
...
{
    printf("=========================================\n");
    printf("       HTTPResponse — Enhetstester\n");
    printf("=========================================\n");
