...
Skriptet använder Pythons inbyggda `sqlite3`-modul och kör samma `CREATE TABLE IF NOT EXISTS` och `ALTER TABLE ADD COLUMN`-migrationer som servern gör. `INSERT OR IGNORE` gör att befintlig konfiguration lämnas orörd.

**Schemamigration:** Det gamla `gridguard.db` saknade kolumnerna `grid_fee_low`, `grid_fee_normal` och `grid_fee_high`. `Database_Initiate()` kör nu `ALTER TABLE ADD COLUMN` för varje kolumn och ignorerar `"duplicate column name"`-felet som SQLite returnerar om kolumnen redan finns.
...
**`LoadScheduler`** hittar det billigaste sammanhängande tidsfönstret för en given last. Sliding window-algoritmen itererar alla möjliga starttider och väljer fönstret med lägst total kostnad, och beräknar även vad samma last kostar om den startar omedelbart.

**`ScheduleDB`** lagrar planerade körningar i `schedules`-tabellen i SQLite. Soft delete — "cancelled" är ett statusvärde, inte en DELETE-SQL.

**Tre nya endpoints i `ClientHandler.c`:**

- `POST /schedule` — tar `load_id`, `duration_minutes`, `power_kw` och `deadline`. Kör hela forecast-pipelinen, hittar optimalt fönster och returnerar `scheduled_start`, `estimated_cost_sek` och `savings_sek`.
- `GET /schedule` — listar alla aktiva scheman för inloggad användare.
- `DELETE /schedule/<id>` — avbokar ett schema.
...
`CompletionRegistry` är en global array som mappar `userId → WorkCompletion*`. Problemet: när compute-tråden signalerade och HTTP-tråden väckte var `WorkCompletion`-objektet redan frigjort på stacken. Nästa request för samma `userId` hittade slot 0 med det gamla `userId` kvar — pointervärdena var skräp.

Lösningen är att anropa `UnregisterCompletion()` innan `WorkCompletion_Signal()`, på alla tre utvägar ur compute-loopen. Det tar bort posten ur registret medan signalen skickas, så nästa request börjar med ett rent tillstånd.
...
- **BUY-tröskeln var meningslös.** `avgTotalCost × 0.80` ger noll BUY-signaler när priserna är jämna och trettio BUY-signaler när de spretar.
- **SELL vid negativt spotpris.** Vid negativa spotpriser betalar man för att mata in på nätet — bättre att hålla IDLE.
- **Totalkostnad i sammanfattningen använde spotpriset** utan nätavgift, skatt och moms — ungefär 40% av vad kunden faktiskt betalar.

### Ny struktur: tre passes

**Pass 1** beräknar totalkostnad (spot + nätavgift + skatt + moms) för varje giltig timme.

**Pass 2** kör `qsort` och deriverar två trösklar: `buyThreshold` (30:e percentilen) och `medianCost` (50:e percentilen).
...
Varje timme innehåller nu `savings_vs_median_sek_kwh` — hur mycket billigare eller dyrare timmen är jämfört med medianen. `summary.total_cost_sek` visar nu faktisk konsumentkostnad med alla avgifter.

---

## Kvarstående begränsningar

Förbrukningsprofilen är fortfarande ett konstant `consumptionKwh` per timme. Verkliga hushåll har morgon- och kvällstoppar som är 2–3× baslasten. Det kräver att kunden anger en timbaserad förbrukningsprofil eller att systemet lär sig mönstret från historik.
