...
TEST_F(RestartPolicyTest, CreateAndCheckLimits) {
    ASSERT_EQ(RestartPolicy_Initiate(&policy, 5, 300, 2), 0);
...
TEST_F(RestartPolicyTest, ExponentialBackoffProgression) {
    RestartPolicy_Initiate(&policy, 5, 300, 2);
    policyInitialized = true;
...
TEST_F(RestartPolicyTest, RateLimitingPreventsRestartStorm) {
    RestartPolicy_Initiate(&policy, 3, 300, 2);
    policyInitialized = true;
...
TEST_F(RestartPolicyTest, CustomBaseBackoff) {
    RestartPolicy_Initiate(&policy, 3, 300, 5);
    policyInitialized = true;
...
TEST_F(RestartPolicyTest, MultiplePoliciesIndependent) {
    RestartPolicy policy1, policy2;
    RestartPolicy_Initiate(&policy1, 3, 300, 2);
    RestartPolicy_Initiate(&policy2, 5, 600, 4);
    ...
    RestartPolicy_Shutdown(&policy1);
    RestartPolicy_Shutdown(&policy2);
}
...
TEST_F(RestartPolicyTest, NullPointerSafety) {
    // RestartPolicy_Shutdown should handle nullptr gracefully
    RestartPolicy_Shutdown(nullptr);

    // RestartPolicy_Initiate should return -1 on nullptr
    EXPECT_EQ(RestartPolicy_Initiate(nullptr, 5, 300, 2), -1);
